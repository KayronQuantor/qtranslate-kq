...
* Compatibility: replaced legacy DOM mutation events with `MutationObserver` and legacy jQuery `bind`/`unbind` calls with `on`/`off` on current WordPress admin screens.
* Compatibility: declared explicit JavaScript dependencies for qTranslate-KQ admin and block-editor bundles.
* Maintenance: removed unconditional widget-save debug logging and hardened request-derived post-type sanitization.
* Compatibility: replaced deprecated `FILTER_SANITIZE_STRING` usage in the dormant ACF helper with WordPress sanitization APIs.
* Fix: the visible title field of WordPress Custom HTML widgets now receives its multilingual UI marker immediately when the field is created, without requiring an initial language switch.

### 4.1.0 - 2026-09-15
* Internal namespace: standardized current QKQ/qkq identifiers to QTKQ/qtkq and renamed active qTranslateX/qtranslatex source identifiers to qTranslateKQ/qtranslatekq (no functional changes).
* Fix: multilingual title editing for WordPress Custom HTML widgets; the visible title field is synchronized with qTranslate-KQ's translated sync field when switching languages.
