...
* See `QTKQ_Admin_Module` for the internal definition of a module and the built-in modules setup.
* The status of the modules is handled by `QTKQ_Admin_Module_Manager` through the `qtranslate_modules_state` option. If this option is not set, the status of the modules is _undefined_ and they are not loaded.
...
* Only the _active_ modules are then loaded by `QTKQ_Module_loader`, both on the client and/or admin side (this depends on the module hooks). Any module in a state other than active is **not** loaded!
...
These are purely preferences handled by `QTKQ_Admin_Module_Settings` and stored in `qtranslate_admin_enabled_modules`.  They don't override the main conditions that prevail for the module activation.
